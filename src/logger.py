...
def logger(message, progress_indicator = False, color = 'default'):
    ...
    # formatted_message_colored = formatted_message + ' \033'
    ...
    if progress_indicator:
        if not last_log_is_progress:
            ...
            formatted_message = "[{}] => {}".format(formatted_datetime, 'Processando ultima ação...') 
            sys.stdout.write(formatted_message)
            sys.stdout.flush()
        else:
            # sys.stdout.write(color_formatted + '.' + conts)
            sys.stdout.write('.')
            sys.stdout.flush()
        return
